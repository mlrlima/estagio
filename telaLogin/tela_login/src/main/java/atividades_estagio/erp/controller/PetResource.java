...
public class PetResource{
	...
	@GET
	public List<Pet> todos(@Context SecurityContext securityContext) {
		Usuario logado=usuarioLogado(securityContext);
		
		List<Pet> todos=pets.todos();
		
		if(logado.getRole()!=Role.ADMIN) {
			todos=pets.filtrarPetsDoUsuario(todos, logado)
		}
	...
	@GET
	@Path("/{id}")
	public Response porId(@PathParam("id") Long id, @Context SecurityContext securityContext) {
		Pet pet=pets.porId(id);
		
		if(pet==null) return Response.status(Response.Status.NOT_FOUND).build();
		
		if (!ehDonoOuAdmin(usuarioLogado(securityContext), pet)) {
			return respostaProibido();
		}
	...
	@GET
	@Path("/usuario/{userId}")
	public Response petsDoUsuario(@PathParam("userId") Long userId, @Context SecurityContext securityContext) {
		Usuario logado = usuarioLogado(securityContext);
		if (logado.getRole() != Role.ADMIN && !logado.getId().equals(usuarioId)) {
			return respostaProibido();
		}
	...
	// usuário comum só cria pet pra si mesmo (dono é forçado = quem está logado)
	// ADMIN pode criar pet pra qualquer usuário, informando "dono":{"id":...}
	@POST
	public Response criar(Pet pet, @Context SecurityContext securityContext) {
		Usuario logado = usuarioLogado(securityContext);
		...
		pet.setId(null); //garante que eh um registro novo
	...
	@PUT
	@Path("/{id}")
	public Response atualizar(@PathParam("id") Long id, Pet pet, @Context SecurityContext securityContext) {
		Pet existente=pets.porId(id);
		
		if(existente==null) return Response.status(Response.Status.NOT_FOUND).build();
		
		Usuario logado = usuarioLogado(securityContext);
		if (!ehDonoOuAdmin(logado, existente)) return respostaProibido();
		...
		if (logado.getRole() == Role.ADMIN) {
			...
			pet.setDono(existente.getDono()); // usuário comum não pode trocar o dono do pet
		}
	...
	@DELETE
	@Path("/{id}")
	public Response excluiur(@PathParam("id") Long id, @Context SecurityContext securityContext){
		Pet pet=pets.porId(id);
	...
	private boolean ehAdminOuDono(Usuario logado, Pet pet) {
